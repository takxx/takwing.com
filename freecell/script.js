...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!board) {
    console.error('FreeCell: #board was not found.');
    return;
  }

  /* ---------- iOS gesture prevention ---------- */

  // This prevents Safari's double-tap fallback where it is cancelable.
  let lastTouchEnd = 0;
  ...
  // These are needed for Safari gesture events.
  ...
  // Prevent native touch behavior inside the game area.
  ...
  function render() {
    ...
    document
      ...
      .forEach((slot, index) => {
        ...
        slot.append(label, document.createTextNode(` ${index + 1}`));
  ...
  function moveFreeToTableau(index, destinationIndex) {
    ...
    if (!card || !canPlaceOnTableau(card, destination)) {
      return false;
    }
  ...
  function onPointerDown(event) {
    ...
    // Foundation cards are not draggable.
    if (!cardElement.dataset.loc) {
      return;
    }

    const source = getPointerSource(cardElement);
  ...
  function onPointerUp(event) {
    ...
    try {
      ...
      // The pointer capture may already have been released.
    }
  ...
  function dropDrag(event) {
    ...
    if (destination) {
      ...
      if (source?.type === 'free') {
        const freeIndex = Number(original.dataset.idx);
  ...
  /*
   * Deliberately no "dblclick" listener.
   * Double-tap is not a game command anymore.
   */
  ...
  autoButton?.addEventListener('click', () => {
    while (autoMoveToHome()) {
      // Keep moving available cards to foundations.
    }
  });
